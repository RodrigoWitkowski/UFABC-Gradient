...
function fillProfile(profile) {
  ...
  const savedView = readStorageJson(rankingViewStorageKey);
  const savedFilters = savedView?.studentId === state.studentId ? savedView.filters : null;
  const hard = savedFilters?.hard_constraints || profile.preferences?.hard_constraints || {};
  const soft = savedFilters?.soft_preferences || profile.preferences?.soft_preferences || {};
...
async function saveProfile() {
  throw new Error("Fluxo legado desativado.");
}
