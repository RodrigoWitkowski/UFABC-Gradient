...
def test_web_interface_and_assets_are_served(client: TestClient) -> None:
    page = client.get("/")
    stylesheet = client.get("/assets/base.css")
    script = client.get("/src/main.js")
    legacy_script = client.get("/src/legacy-app.js")

    assert page.status_code == 200
    assert "Gradient" in page.text
    assert 'base.css?v=' in page.text
    assert 'src/main.js?v=' in page.text
    ...
    assert '@import "./layout.css"' in stylesheet.text
    assert script.status_code == 200
    assert "GradientModules" in script.text
    assert legacy_script.status_code == 200
    assert 'fetchJson("/rankings/sections"' in legacy_script.text
    assert "gradient_ranking_view_v2" in legacy_script.text
    assert "result_limit: maxRankingResults" in legacy_script.text
    assert 'data-results-action="more"' in legacy_script.text
    assert "compatibilidade" in legacy_script.text
    assert "disciplinas sendo cursadas atualmente" in legacy_script.text
...
def test_offer_import_and_status_endpoints(
    ...
    status_response = client.get(f"/admin/imports/{payload['id']}")
...
def test_curriculum_api_keeps_category_on_relationship(client: TestClient) -> None:
    ...
    list_response = client.get("/admin/courses")
    assert list_response.status_code == 200
    assert list_response.json()[0]["curriculum_versions"][0]["version"] == "2025"
    get_response = client.get(f"/admin/courses/{course_id}/curriculums/2025")
...
def test_student_academic_profile_api(client: TestClient, session: Session) -> None:
    ...
    update_response = client.put(
        f"/admin/students/{student_id}/academic-profile",
        json={
            "ra": "11234567890",
            "admission_year": 2025,
            "admission_shift": "Noturno",
    ...
    classification_response = client.get(
        f"/admin/students/{student_id}/subjects/MCCC001-23/classifications"
    )
...
def test_history_backed_profile_rejects_manual_academic_edits(
    ...
    response = client.put(
        f"/admin/students/{student['id']}/academic-profile",
        json={
            "ra": student["ra"],
